...
class FlexibleJobShopDataHandler:
    ...
    def _load_data(self) -> None:
        ...
        if self.data_type == "dataset":
            ...
            if isinstance(self.data_source, dict):
                # Add seed to simulation parameters
                simulation_params = self.data_source.copy()
                if self.seed is not None:
                    simulation_params['seed'] = self.seed
                jobs, self.operations, self.num_jobs, self.num_machines, self.num_operations = \
                    DataGenerator.generate_synthetic_data(**simulation_params)
                self.jobs = {job.job_id: job for job in jobs}
            else:
    ...
    def get_total_weight(self) -> int:
        """Get total weight for all jobs."""
        return sum(self.get_jobs_weight())

    def get_max_due_date(self) -> int:
        """Get max due date for all jobs."""
        return max(self.get_jobs_due_date())
    
    def get_total_weighted_tardiness(self, completion_times: Dict[int, int]) -> float:
        """
        ...
        Returns:
            Normalized total weighted tardiness
        """
        total_twt = 0
        for job in self.jobs.values():
