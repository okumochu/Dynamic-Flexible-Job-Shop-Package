"""
Generalization Experiment for Flat vs Hierarchical RL with/without Curriculum Learning - SPARSE REWARDS
Tests the generalization capability and effectiveness of curriculum learning on Dynamic Flexible Job Shop Scheduling
This version uses sparse rewards (use_reward_shaping = False)
...
def run_generalization_experiment(training_envs: int, test_envs: int, epochs: int = 20):
    """Main experiment function testing both curriculum and non-curriculum approaches with SPARSE REWARDS"""
    print("Starting Comprehensive Generalization Experiment - SPARSE REWARDS...")
    print("Testing: Curriculum Learning vs Single Environment Training (use_reward_shaping = False)")
    print("="*70)
    ...
    # Convert to RL environments with SPARSE REWARDS (use_reward_shaping=False)
    training_rl_envs = []
    for data_handler in training_environments:
        rl_env = RLEnv(data_handler, alpha=config.common_rl_params['alpha'], 
                      use_reward_shaping=False)  # SPARSE REWARDS
        training_rl_envs.append(rl_env)
    
    # Setup directories with different names to avoid conflicts
    flat_config = config.get_flat_rl_config()
    hierarchical_config = config.get_hierarchical_rl_config()
    
    # Modify directory names to include "sparse_reward" suffix
    flat_config['result_dirs']['training_process'] = flat_config['result_dirs']['training_process'].replace('flat_rl', 'flat_rl_sparse_reward')
    flat_config['result_dirs']['model'] = flat_config['result_dirs']['model'].replace('flat_rl', 'flat_rl_sparse_reward')
    
    hierarchical_config['result_dirs']['training_process'] = hierarchical_config['result_dirs']['training_process'].replace('hierarchical_rl', 'hierarchical_rl_sparse_reward')
    hierarchical_config['result_dirs']['model'] = hierarchical_config['result_dirs']['model'].replace('hierarchical_rl', 'hierarchical_rl_sparse_reward')
    ...
    print("\n" + "="*70)
    print("EXPERIMENT 1: FLAT RL WITH CURRICULUM LEARNING (SPARSE REWARDS)")
    print("="*70)
    ...
    print("\n" + "="*70)
    print("EXPERIMENT 2: FLAT RL WITH SINGLE ENVIRONMENT (SPARSE REWARDS)")
    print("="*70)
    ...
    print("\n" + "="*70)
    print("EXPERIMENT 3: HIERARCHICAL RL WITH CURRICULUM LEARNING (SPARSE REWARDS)")
    print("="*70)
    ...
    print("\n" + "="*70)
    print("EXPERIMENT 4: HIERARCHICAL RL WITH SINGLE ENVIRONMENT (SPARSE REWARDS)")
    print("="*70)
    ...
    # ============= RESULTS ANALYSIS =============
    print("\n" + "="*70)
    print("RESULTS ANALYSIS - SPARSE REWARDS")
    print("="*70)
    ...
    print("\n📊 TRAINING PERFORMANCE (2x2 Table) - SPARSE REWARDS")
    print("="*70)
    print(training_table.to_string(index=False))
    
    print("\n📊 TESTING PERFORMANCE (2x2 Table) - SPARSE REWARDS")
    print("="*70)
    print(testing_table.to_string(index=False))
    
    # Save results to CSV with different names
    training_csv_path = 'training_performance_results_sparse_reward.csv'
    testing_csv_path = 'testing_performance_results_sparse_reward.csv'
    
    training_table.to_csv(training_csv_path, index=False)
    testing_table.to_csv(testing_csv_path, index=False)
    
    print(f"\n💾 SPARSE REWARD Results saved to:")
    print(f"   Training performance: {training_csv_path}")
    print(f"   Testing performance: {testing_csv_path}")
    
    print("\n✅ SPARSE REWARD Experiment completed successfully!")
...
if __name__ == "__main__":
    ...
    # Run the experiment with 800 epochs for full training - SPARSE REWARDS
